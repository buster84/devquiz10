...
$count = array();
for( $h = 1; $h <= $level1->height; $h++ ){
  for( $w = 1; $w <= $level1->width; $w++ ){
    if( !isset( $count[count( $level1->map[$w][$h]['can_move'] )] ) ){
      $count[count( $level1->map[$w][$h]['can_move'] )] = 0;
    }
    $count[count( $level1->map[$w][$h]['can_move'] )]++;
  }
}
