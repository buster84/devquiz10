...
while( 1 ){
  ...
  if( 100 === $debug_i ){
    ...
    echo "time: $time";
    echo "debug:";
    echo " " . implode( "", $level1->my->select ). "\n";
  }

  $select_i = 0;
  $can_command = array();
  foreach( $level1->map[$level1->my->w][$level1->my->h]['can_way'] as $data ){
    if( array( $level1->my->before_w, $level1->my->before_h ) === $data['can_move'] ){
      $back_command = $data['can_command'];
      continue;
    }
    $can_command[] = $data['can_command'];
  }
  if( empty( $can_command ) ){
    $can_command[] = $back_command;
  }

  if( 1 === $backflg ){
    if( empty( $level1->my->select ) ){
      break;
    }
    $last_select = array_pop( $level1->my->select );
    $tmp_select  = $level1->my->select;
    $level1->reset();
    foreach( $tmp_select as $str ){
      $level1->run( $str );
    }
    if( $time < $level1->time ){
      $backflg = 1;
    ...
    //echo "w:" . $level1->my->w . " h:" . $level1->my->h . "\n";
    //var_dump( $can_command );
    $can_command = array();
    foreach( $level1->map[$level1->my->w][$level1->my->h]['can_way'] as $data ){
      if( array( $level1->my->before_w, $level1->my->before_h ) === $data['can_move'] ){
        $back_command = $data['can_command'];
    ...
    foreach( $can_command as $i => $command ){
      if( $last_select === $command ){
        if( !isset( $can_command[$i + 1] ) ){
          $backflg = 1;
          continue 2;
        } else {
          $select_i = $i + 1;
  ...
  // echo "no. $select_i \n";
  $level1->run( $can_command[$select_i] );
  //$level1->run( $level1->map[$level1->my->w][$level1->my->h]['can_way'][$select_i]['can_command'] );

  if( 1 === $level1->finish_flg ){
    if( 1 === $level1->clear_flg ){
      if( $level1->time < $time ){
        $time = $level1->time;
      }     
      echo $level1->limit_time - $level1->time; 
      echo " " . implode( "", $level1->my->select ). "\n";
    } 
    //echo " " . implode( "", $level1->my->select ). "\n";
    $backflg = 1;
